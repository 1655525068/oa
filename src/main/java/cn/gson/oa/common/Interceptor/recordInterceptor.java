package cn.gson.oa.common.Interceptor;

import cn.gson.oa.common.Tool;
import cn.gson.oa.model.dao.roledao.RolepowerlistDao;
import cn.gson.oa.model.dao.system.SystemMenuDao;
...
//		uLog.setUser(new User());
        ...
        for (SystemMenu systemMenu : sMenus) {
            if (systemMenu.getMenuUrl().equals(request.getServletPath())) {
                //只有当该记录的路径不等于第一条的时候
				UserLog userLog = userLogDao.findByUserlaset(id);
                if (userLog == null || !userLogDao.findByUserlaset(id).getUrl().equals(systemMenu.getMenuUrl())) {
