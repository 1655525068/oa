...
public interface UserLogDao extends JpaRepository<UserLog, Long> {
    ...
    // 无限制
    @Query(nativeQuery = true, value = ("SELECT * from aoa_user_log where aoa_user_log.user_id=?1 ORDER BY aoa_user_log.log_time DESC"))
    List<UserLog> findByUser2(long userid);
